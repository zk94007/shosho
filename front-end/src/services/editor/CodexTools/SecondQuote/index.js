...
export default class SecondQuoteTool {
  ...
  set state(state) {
    ...
    if (state) $(this.button).prev().addClass(this.api.styles.inlineToolButtonActive);
  }
  ...
  render() {
    ...
    this.button.classList.add(this.api.styles.inlineToolButton, 'ce-inline-tool--secondQuote');
    return this.button;
  }
  ...
  surround() {
    if (this.state) {
      ...
      this.api.events.emit('inline-quote', {index: this.api.blocks.getCurrentBlockIndex(), type: 'secondQuote'});
    }
  ...
  checkState(selection) {
    const mark = $(selection.focusNode).closest('div').hasClass('ce-secondquote')
    this.state = !!mark;
  }
}